...
def home():
    ...
    y = x.replace(day=x.day, hour=0, minute=0, second=0,
    ...
    def set_daily_capacity():
        for doctor in Doctor.query.all():
            if doctor.status == 1:
                doctor.daily_capacity = 10
        db.session.commit()

    t = Timer(secs, set_daily_capacity)
    t.start()
    if session.get("NC"):
...
def login():
    ...
    if not request.form['user'] or not request.form['password']:
        ...
        try:
            ...
            flash(
                'نام کاربری وارد شده صحیح نیست ، نام کاربری کد پرسنلی شما می باشد', category='danger')
            return redirect(url_for('login_page'))
        ...
        if request.form['NC']:
            try:
                ...
                if len(str(nc)) == 10:
                    ...
                    flash(
                        "کد ملی وارد شده صحیح نیست کد ملی باید دقیقا 10 رقم عددی باشد", category='danger')
            except:
                flash(
                    "کد ملی وارد شده صحیح نیست کد ملی باید دقیقا 10 رقم عددی باشد", category='danger')
        else:
...
def admin_login(input):
    ...
    if current_user.access_level == 1:
        ...
        if input == 'city':
            ...
            for doctor in Doctor.query.all():
                if doctor.status == 1:
                    medical = Medical.query.filter_by(
                        id=doctor.medical).first()
            ...
            for medical in Medical.query.all():
                if medical.status == 1:
                    city = City.query.filter_by(id=medical.city).first()
                    medicals.append(
                        {"medical_id": medical.id, "medical_name": medical.name + "-" + city.name})
            return render_template('admin-' + input + '.html', values=locals())
        elif input == 'user':
            data.clear()
            ...
            for turn in Turn.query.all():
                if turn.date.date() == datetime.date.today():
                    if turn.status == 1:
                        ...
                        doctor = Doctor.query.filter_by(id=turn.doctor).first()
                        medical = Medical.query.filter_by(
                            id=turn.medical).first()
                        city = City.query.filter_by(id=medical.city).first()
                        specialty = Specialty.query.filter_by(
                            id=doctor.specialty).first()
        ...
        if request.method == 'POST':
            if not request.form['name']:
                ...
                if not any(char.isdigit() for char in request.form['name']):
                    ...
                    flash(
                        'نام شهر وارد شده صحیح نیست. نام شهر نباید شامل عدد باشد', category='danger')
                    return redirect(url_for('admin_login', input='city'))
        return redirect(url_for('admin_login', input='city'))
    else:
        return redirect(url_for('admin_login', input='panel'))
        ...
        if request.method == 'POST':
            if not request.form['name']:
                ...
                if not any(char.isdigit() for char in request.form['name']):
                    ...
                    flash(
                        'نام تخصص وارد شده صحیح نیست. نام تخصص نباید شامل عدد باشد', category='danger')
                    return redirect(url_for('admin_login', input='speciality'))
        return redirect(url_for('admin_login', input='speciality'))
    else:
        return redirect(url_for('admin_login', input='panel'))
    ...
    if current_user.access_level == 1:
        if request.method == 'POST':
            if not request.form['name'] or any(char.isdigit() for char in request.form['name']) \
                ...
                flash(
                    'پر کردن تمام فیلد ها ضروری است. نام نباید شامل عدد باشد', category='danger')
            ...
            if not request.form['name'] or any(char.isdigit() for char in request.form['name']) \
                ...
                try:
                    ...
                    try:
                        ...
                        try:
                            d_capacity = int(
                                request.form['daily_capacity'])
                            if len(str(NC)) == 10:
                                if len(str(phone)) == 10:
                                    if len(str(d_capacity)) <= 50:
                                        doctor = Doctor(name=request.form['name'],
                                                        ...
                                                        daily_capacity=d_capacity)
                                        db.session.add(doctor)
                                        db.session.commit()
                                        flash(
                                            'پزشک با موفقت اضافه شد', 'success')
                                        return redirect(url_for('admin_login', input='doctor'))
                                    else:
                                        flash(
                                ...
                                flash(
                                    'کدملی وارد شده صحیح نیست . کدملی باید دقیقا 10 رقم عددی باشد', category='danger')
                                return redirect(url_for('admin_login', input='doctor'))
        ...
        if request.method == 'POST':
            if not request.form['id'] \
                ...
                try:
                                ...
                                if len(str(NC)) == 10:
                                    if len(str(phone)) == 10:
                                        ...
                                        if Employee.query.filter_by(id=int(user_id)).first():
                                            new_employee = Employee.query.filter_by(
                                            ...
                                            if request.form["position"] == '2' or request.form["position"] == '3':
                                                if request.form['medical'] != '0':
                                                    ...
                                                    new_employee.medical = int(
                                                        request.form["medical"])
                                                    new_employee.position = int(
                                                        request.form["position"])
                                        ...
                                        db.session.commit()
                                        flash(
                                            'کارمند با موفقت اضافه شد', 'success')
                                        return redirect(url_for('admin_login', input='user'))
                                    else:
                                        flash(
                                            'شماره تلفن وارد شده صحیح نیست. شماره را بدون +98 وارد کرده و شماره تلفن ثابت را نیز '
                    ...
                    flash(
                        'کدپرسنلی وارد شده صحیح نیست . کدپرسنلی باید دقیقا 10 رقم عددی باشد', category='danger')
                    return redirect(url_for('admin_login', input='user'))
        return redirect(url_for('admin_login', input='user'))
    else:
        return redirect(url_for('admin_login', input='panel'))
                ...
                if session.get("NC"):
                    ...
                    if len(str(phone)) == 10:
                        doctor = Doctor.query.filter_by(
                            id=int(request.form['doctor'])).first()
                        if doctor.daily_capacity > 0:
                            sick = Sick.query.filter_by(
                                NC=session.get("NC")).first()
                            ...
                            flash(
                                'میتوانید از صفحه پیگیری نویت ، نوبت های خود را پیگیری کنید', category='success')
                            return redirect(url_for('panel'))
                        else:
                            flash(
                                'تعداد نوبت روزانه پزشک موردنظر به پایان رسیده است', category='danger')
                            return redirect(url_for('user_turn'))
                    else:
                        flash(
                            'شماره تلفن وارد شده صحیح نیست. شماره را بدون +98 وارد کرده و شماره تلفن ثابت را نیز با 076 '
            ...
            if not request.form["new_name"] \
                ...
                try:
                    ...
                    if len(str(new_phone)) == 10:
                        new_medical = Medical.query.filter_by(
                            id=int(medical_id)).first()
                        ...
                        if not new_medical == comparison_medical:
                            ...
                            db.session.commit()
                            flash('اطلاعات جدید موفقیت ثبت شد',
                                  category='success')
                            return redirect(url_for('admin_login', input='medical'))
    ...
    if current_user.access_level == 1:
        if request.method == "POST":
            if not request.form["new_name"] or any(char.isdigit() for char in request.form['new_name']):
                flash(
                    'پرکردن فیلد نام شهر الزامی است و نام شهر نباید شامل عدد باشد', category='danger')
                return redirect(url_for("admin_login", input="city"))
            else:
...
def delete_speciality(speciality_id):
    if current_user.access_level == 1:
        if request.method == "POST":
            speciality = Specialty.query.filter_by(
                id=int(speciality_id)).first()
            speciality.status = 0
...
def edit_speciality(speciality_id):
    if current_user.access_level == 1:
        if request.method == "POST":
            if not request.form["new_name"] or any(char.isdigit() for char in request.form['new_name']):
                flash(
                    'پرکردن نام فیلد تخصص الزامی است و نام تخصص نباید شامل عدد باشد', category='danger')
                return redirect(url_for("admin_login", input="speciality"))
            else:
                new_speciality = Specialty.query.filter_by(
                    id=int(speciality_id)).first()
        ...
        if request.method == 'POST':
            if not request.form['new_name'] or any(char.isdigit() for char in request.form['new_name']) \
                ...
                char.isdigit() for char in request.form['new_last_name']) \
                    ...
                    or not request.form['new_phone'] \
                    or not request.form['new_d_capacity'] \
                ...
                try:
                    ...
                    try:
                        ...
                        if len(str(NC)) == 10:
                            if len(str(phone)) == 10:
                                new_d_capacity = int(
                                    request.form['new_d_capacity'])
                                if len(str(new_d_capacity)) <= 50:
                                    new_doctor = Doctor.query.filter_by(
                                        id=int(doctor_id)).first()
                                    ...
                                    if not new_doctor == comparison_doctor:
                                        ...
                                        new_doctor.medical = int(
                                            request.form["new_medical"])
                                        new_doctor.specialty = int(
                                            request.form["new_specialty"])
                                        new_doctor.daily_capacity = new_d_capacity
                                        db.session.commit()
                                ...
                                if request.form["new_position"] == '2' or request.form["new_position"] == '3':
                                    if request.form['new_medical'] != "0":
                                        ...
                                        if not new_employee == comparison_employee:
                                            ...
                                            db.session.commit()
                                            flash(
                                                'اطلاعات جدید با موفقیت ثبت شد', 'success')
                                            return redirect(url_for('admin_login', input='user'))
                                    else:
            ...
            if check_password_hash(current_user.password, request.form["old_password"]):
                if len(request.form["new_password"]) >= 8:
                    if request.form["new_password"] == request.form["confirm_password"]:
                        ...
                        flash('رمز عبور جدید و تکرار آن یکی نیست',
                              category='danger')
                        return redirect(url_for("admin_login", input="changepassword"))
                else:
            ...
            try:
                ...
                try:
                    ...
                    if len(str(new_NC)) == 10:
                        if len(str(new_phone)) == 10:
                            new_sick = Sick.query.filter_by(
                                id=int(sick_id)).first()
                            ...
                            if not new_sick == comparison_sick:
                                ...
                                db.session.commit()
                                is_search = False
                                flash('اطلاعات بیمار بروزرسانی شد',
                                      category='success')
                                return redirect(url_for("admin_login", input="reception"))
                        else:
                            flash(
                        ...
                        flash('کد ملی باید دقیقا 10 رقم عددی باشد',
                              category='danger')
                        return redirect(url_for("admin_login", input="reception"))
                except:
                    flash(
        ...
        if request.method == "POST":
            ...
            if not request.form["start_date"] or not request.form["end_date"]:
                ...
                start_date = datetime.datetime(
                    int(s_date[0]), int(s_date[1]), int(s_date[2]))
                end_date = datetime.datetime(
                    int(e_date[0]), int(e_date[1]), int(e_date[2]))
                ...
                filter_by = {}
                filter_by = {"city": city_id, "medical": medical_id,
                             "sp": speciality_id, "doctor": doctor_id}
                for i in filter_by.copy():
                    if not filter_by[i]:
                        del filter_by[i]
                select_sp = False
                result = Turn.query.filter(
                    Turn.date >= start_date, Turn.date <= end_date)
                for k, v in filter_by.items():
                    if k == "city":
                        result = result.join(Medical).join(
                            City).filter(City.id == city_id)
                    if k == "medical":
                        result = result.filter(Turn.medical == medical_id)
                    if k == "doctor":
                        result = result.filter(Turn.doctor == doctor_id)
                    if k == "sp":
                        result = result.join(Doctor, Turn.doctor == Doctor.id).join(
                            Specialty, Doctor.specialty == Specialty.id).filter(Specialty.id == speciality_id)
                        select_sp = True
